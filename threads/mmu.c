...
/* 페이지 맵 레벨 4, pml4, 내에서 가상 주소 VADDR에 대한 페이지 테이블 엔트리의 주소를 반환합니다. 
 * PML4 엔트리가 VADDR에 대한 페이지 테이블을 가지고 있지 않은 경우,
 * CREATE 매개변수에 따라 동작이 달라집니다.
 * CREATE가 true로 설정된 경우, 새로운 페이지 테이블이 생성되고 해당 테이블로의 포인터가 반환됩니다.
...
/*
 * 커널 가상 주소에 대한 매핑을 갖고 있는
 * 새로운 페이지 맵 레벨 4 (pml4)를 생성하며,
 * 사용자 가상 주소에 대한 매핑은 갖고 있지 않습니다.
 * 메모리 할당이 실패한 경우,
 * 새로운 페이지 디렉터리를 반환하지 않고 널 포인터를 반환합니다.
 */
 ...
 * KPAGE는 아마도 `palloc_get_page()`를 사용하여 사용자 풀에서 얻은 페이지일 것입니다.
 * 만약 WRITABLE이 true로 설정된 경우,
 * 새 페이지는 읽기/쓰기 가능하며, 그렇지 않으면 읽기 전용입니다.
