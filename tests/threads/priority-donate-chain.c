...
static void func_low(struct lock_pair *locks) {
  lock_acquire(locks->second);  // 39로 오름
  msg("low got lock b, priority: %d", thread_get_priority());
  lock_acquire(locks->first);  // main에서 대기
  msg("low got lock a, priority: %d", thread_get_priority());
  lock_release(locks->second); //lock b 릴리즈
  msg("low release lock b priority: %d", thread_get_priority());
  lock_release(locks->first); //lock b 릴리즈
  msg("low release lock a priority: %d", thread_get_priority());
  
}

static void func_mid(struct lock_pair *locks) {
  msg("mid acquire lock a, priority: %d", thread_get_priority());
  lock_acquire(locks->first);
  msg("mid got lock a, priority: %d", thread_get_priority());
}

static void func_mid_low(struct lock_pair *locks) {
  msg("mid_low RUN!!! priority: %d", thread_get_priority());
}

static void func_high(struct lock_pair *locks) {
  msg("high acquire lock b, priority: %d", thread_get_priority());
  lock_acquire(locks->second);  // low 39
...
void test_priority_donate_chain(void) {
  ...
  // thread_create("mid_low", PRI_DEFAULT + 2, func_mid_low,NULL);
  ...
  // msg("end");
    ...
    for (i = 1; i < NESTING_DEPTH; i++) {
      ...
      thread_create(name, thread_priority, donor_thread_func, lock_pairs +
      i); msg("%s should have priority %d.  Actual priority: %d.",
      thread_name(),
